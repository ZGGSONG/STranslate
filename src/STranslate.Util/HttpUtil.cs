...
public class HttpUtil
{
    ...
    public static async Task PostAsync(Uri uri, string req, string? key, Action<string> OnDataReceived, CancellationToken token, int timeout = 10)
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Post, uri) { Content = new StringContent(req, Encoding.UTF8, "application/json") };
    ...
    private static HttpRequestMessage CreateHttpRequestMessage(HttpMethod method, string url, string? content, Dictionary<string, string>? headers)
    {
