...
internal struct MultiProxy
{
    ...
    /// <param name="secure">
    ///     If true, return proxies suitable for use with a secure connection. If false, return proxies
    ///     suitable for an insecure connection.
    /// </param>
    public static MultiProxy Parse(FailedProxyCache failedProxyCache, string? proxyConfig, bool secure)
    {
        ...
        while (TryParseProxyConfigPart(span, secure, out var uri, out var charactersConsumed))
        {
            var idx = uris.Length;
    ...
    public bool ReadNext([NotNullWhen(true)] out Uri? uri, out bool isFinalProxy)
    {
        ...
        var oldestFailedProxyTicks = long.MaxValue;

        do
        {
            Debug.Assert(_failedProxyCache != null);
            var renewTicks = _failedProxyCache.GetProxyRenewTicks(uri);
    ...
    private bool ReadNextHelper([NotNullWhen(true)] out Uri? uri, out bool isFinalProxy)
    {
        ...
        {
            var hasProxy = TryParseProxyConfigPart(_proxyConfig.AsSpan(_currentIndex), _secure, out uri!,
    ...
    )
    {
        ...
        var wantedFlag = secure ? SECURE_FLAG : INSECURE_FLAG;
        var originalLength = proxyString.Length;

        while (true)
        {
            // Skip any delimiters.
            var iter = 0;
            while (iter < proxyString.Length && Array.IndexOf(s_proxyDelimiters, proxyString[iter]) >= 0) ++iter;

            if (iter == proxyString.Length) break;

            proxyString = proxyString.Slice(iter);

            // Determine which scheme this part is for.
            // If no schema is defined, use both.
            var proxyType = SECURE_FLAG | INSECURE_FLAG;
            ...
            if (iter < 0) iter = proxyString.Length;

            // Return URI if it's a match to what we want.
            if (
                (proxyType & wantedFlag) != 0
                && Uri.TryCreate(string.Concat("http://", proxyString.Slice(0, iter)), UriKind.Absolute, out uri)
            )
            {
