...
internal sealed class FailedProxyCache
{
    ...
    private SpinLock _flushLock = new(false); // mutable struct; do not make this readonly
    ...
    /// <returns>
    ///     If the proxy can be used, <see cref="Immediate" />. Otherwise, the next <see cref="Environment.TickCount64" />
    ...
    public long GetProxyRenewTicks(Uri uri)
    {
        ...
        if (!_failedProxies.TryGetValue(uri, out var renewTicks)) return Immediate;

        // If we haven't reached out renew time, the proxy can't be used.
        if (Environment.TickCount64 < renewTicks) return renewTicks;

        // Renew time reached, we can remove the proxy from the cache.
        if (TryRenewProxy(uri, renewTicks)) return Immediate;
    ...
    /// <summary>
    ///     Renews a proxy prior to its period expiring. Used when all proxies are failed to renew the proxy closest to being
    ///     renewed.
    /// </summary>
    /// <param name="uri">The <paramref name="uri" /> of the proxy to renew.</param>
    /// <param name="renewTicks">
    ///     The current renewal time for the proxy. If the value has changed from this, the proxy will not
    ///     be renewed.
    /// </param>
    ...
    {
        if (_failedProxies.Count > LargeProxyConfigBoundary &&
            Environment.TickCount64 >= Interlocked.Read(ref _nextFlushTicks)) CleanupHelper();
    }
    ...
    ///     so it's only run periodically and is disabled until we exceed <see cref="LargeProxyConfigBoundary" /> failed
    ///     proxies.
    /// </remarks>
    ...
    private void CleanupHelper()
    {
        ...
        try
        {
            ...
            foreach (var kvp in _failedProxies)
