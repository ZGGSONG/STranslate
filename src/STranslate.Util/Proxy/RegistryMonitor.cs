...
namespace STranslate.Util.Proxy
{
    ...
    internal sealed class RegistryMonitor : IDisposable
    {
        ...
        private RegChangeNotifyFilter _regFilter =
            RegChangeNotifyFilter.Key | RegChangeNotifyFilter.Attribute | RegChangeNotifyFilter.Value | RegChangeNotifyFilter.Security;
        ...
        public void Start()
        {
            ...
            lock (_threadLock)
            {
                if (!IsMonitoring)
                {
                    ...
                    _thread = new Thread(new ThreadStart(MonitorThread)) { Name = "RegistryMonitor", IsBackground = true };
                    _thread.Start();
                }
            }
        }
