...
{
    public partial class TranslatorOllama : ObservableObject, ITranslator
    {
        #region Constructor

        public TranslatorOllama() : this(Guid.NewGuid(), "http://localhost:11443", "Ollama")
        {
        }

        public TranslatorOllama(Guid guid, string url, string name = "", IconType icon = IconType.Ollama, string appID = "", string appKey = "", bool isEnabled = true, ServiceType type = ServiceType.OllamaService)
        {
            Identify = guid;
            Url = url;
            Name = name;
        ...
        public async Task TranslateAsync(object request, Action<string> OnDataReceived, CancellationToken token)
        {
            ...
            if (!uriBuilder.Path.EndsWith("/api/chat"))
            {
