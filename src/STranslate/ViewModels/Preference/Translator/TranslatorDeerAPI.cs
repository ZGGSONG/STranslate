...
public partial class TranslatorDeerAPI : TranslatorLLMBase, ITranslatorLLM
{
    #region Constructor

    public TranslatorDeerAPI()
        : this(Guid.NewGuid(), "https://api.deerapi.com", "DeerAPI")
    {
    }
    ...
    public TranslatorDeerAPI(
        ...
        string url,
        string name = "",
        IconType icon = IconType.DeerAPI,
        string appID = "",
        string appKey = "",
        bool isEnabled = true,
        ServiceType type = ServiceType.DeerAPIService
    )
    {
        Identify = guid;
        Url = url;
    ...
    public async Task TranslateAsync(object request, Action<string> onDataReceived, CancellationToken token)
    {
        ...
        a_model = string.IsNullOrEmpty(a_model) ? "gpt-4o" : a_model;
