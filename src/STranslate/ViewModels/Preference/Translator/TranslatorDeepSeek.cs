...
public partial class TranslatorDeepSeek : TranslatorBase, ITranslatorLlm
{
    #region Constructor
    ...
    [JsonIgnore][ObservableProperty] private IconType _icon = IconType.DeepSeek;
    ...
    {
        var dialog = new PromptDialog(ServiceType.DeepSeekService, (UserDefinePrompt)userDefinePrompt.Clone());
    ...
    private void AddPrompt()
    {
        ...
        var dialog = new PromptDialog(ServiceType.DeepSeekService, userDefinePrompt);
        if (!(dialog.ShowDialog() ?? false)) return;
        var tmp = ((PromptViewModel)dialog.DataContext).UserDefinePrompt;
        userDefinePrompt.Name = tmp.Name;
        userDefinePrompt.Prompts = tmp.Prompts;
