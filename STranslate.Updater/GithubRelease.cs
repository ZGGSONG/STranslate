        ...
        public async Task<VersionInfo?> GetRequest()
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var data = JsonConvert.DeserializeObject<GithubModel>(body) ?? throw new Exception($"Response Error, body is {body}");

                    Info.IsPre = data.Prerelease;
                    Info.Title = data.Name;
                    Info.Version = data.TagName;
                    Info.DownloadUrl = data.Assets?.FirstOrDefault()?.BrowserDownloadUrl ?? throw new Exception($"BrowserDownloadUrl is empty");
